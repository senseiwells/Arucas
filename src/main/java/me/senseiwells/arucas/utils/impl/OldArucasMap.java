...
public class OldArucasMap implements ValueIdentifier {
	...
	public OldArucasMap() {
		this.mask = OldArucasMap.INITIAL_SIZE;
		this.table = new Node[this.mask];
		this.size = 0;
	}
	
	public OldArucasMap(Context context, OldArucasMap map) throws CodeError {
		this();
	 ...
	 */
	public void putAll(Context context, OldArucasMap map) throws CodeError {
	...
	private synchronized int hash(int h) {
		return (h ^ (h >>> 16)) & OldArucasMap.HASH_BITS;
	}
	...
	public boolean isEquals(Context context, Value<?> other) throws CodeError {
		if (!(other.value instanceof OldArucasMap that)) {
			return false;
		}
