...
public class IfNode extends Node {
    public final List<ThreeValues<Node, Node, Boolean>> nodes;
    public final TwoValues<Node, Boolean> elseNode;
    ...
    public Value<?> visit(Interpreter interpreter, Context context) throws CodeError, ThrowValue {
        for (ThreeValues<Node, Node, Boolean> nodes : this.nodes) {
            ...
            if (booleanValue.value) {
                ...
                return nodes.getValue3() ? new NullValue().setContext(context) : value;
            }
        }
        
        if (this.elseNode != null) {
            Value<?> value = interpreter.visit(this.elseNode.getValue1(), context);
            return elseNode.getValue2() ? new NullValue().setContext(context) : value;
        }
