...
public class UnaryOperatorNode extends Node {
    ...
    public Value<?> visit(Interpreter interpreter, Context context) throws Error, ThrowValue {
        ...
        try {
            ...
            else if (this.token.type == Token.Type.NOT)
                value = ((BooleanValue) value).not();
            value.setPos(node.startPos, node.endPos);
            return value;
        }
        catch (ClassCastException classCastException) {
            throw new Error(Error.ErrorType.ILLEGAL_OPERATION_ERROR, "The operation '" + this.token.type + "' cannot be applied to '" + value.value + "'", this.startPos, this.endPos);
        }
    }

    @Override
    public String toString() {
        return "(%s, %s)".formatted(token, node);
    }
}
